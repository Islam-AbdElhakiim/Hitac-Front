...
export const getServerSideProps = async ({ locale }: any) => {
    const data = await getAllPalletss();
    return {
        props: {
            fulfilleds: data,
                                            ...
                                            if (index >= startingIndex && index < currentPage * 10) {
                                                return (
                                                    <tr key={row._id} className=" text-left h-full">
                                                        ...
                                                        <td title={row.supplier?.name}>{row?.supplier?.firstName} {row?.supplier?.lastName}</td>
                                                        <td title={`${row.station?.englishName}`}>
                                                            {row?.station?.englishName}
                                                        </td>
                                                        <td title={row.packingDate}>{new Date(row?.packingDate).toLocaleDateString()}</td>

...
import { returnRequestsType, supplyOrderType } from "@/types";
import {
...
export default function ReturnRequests({ orders }: any) {
  ...
  const handleDelete = async () => {
    const deletes = async () => {
      ...
      try {
        ...
        setPageRows((prev: returnRequestsType[]) =>
    ...
    setModalBody(
      "Deleteing the selected return request/s will ERASE THEM FOREVER from the database! "
    );
  ...
  const navigate = (id: any) => {
    router.push(`/return-requests/${id}`);
  };
  return (
    <>
      {isLoading ? (
        ...
        <div className="flex flex-col justify-center items-center px-10 ">
          <PageHeader pageTitle="pages.return-requests" />
          {/* Page Body */}
            ...
            <div className="w-full h-[80%] overflow-auto">
              <table className={` w-full`}>
                ...
                <tbody className="  h-[200px] border border-green-500 overflow-auto">
                  {pageRows
                    ?.filter((order: returnRequestsType) => !order.isDeleted)
                    .map((order: returnRequestsType, index: number) => {
                      if (index >= startingIndex && index < currentPage * 10) {
                        return (
                          <tr key={order._id} className=" text-left h-full">
                            ...
                            <td
                              onClick={() => navigate(order._id)}
                            >{`${order.supplier.firstName} ${order.supplier.lastName}`}</td>
                            <td
                              onClick={() => navigate(order._id)}
                            >{`${order.createdOn}`}</td>
                            <td

...
import { login, postRequest } from "@/http/requests";
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const response: any = await fetch(
        "http://localhost:3002/employees/login",
        {
          method: "POST",
          headers: {
      ...
      //   const response: any = await login("employees/login", {
      //     email,
      //     password,
      //     rememberme,
      //   });
          ...
          <div className=" w-screen h-screen bg-[rgba(0,0,0,0.4)] absolute">
            ...
            <div className="flex flex-col justify-start items-start rounded-3xl bg-bgGray h-[630px] w-[600px] absolute right-[5%] top-[20%] shadow-xl p-14">
              ...
              <form
                ...
                className="flex flex-col items-start justify-center mt-5 w-full"
              >
                <h2 className="font-semibold text-[30px]">Welcome Back!</h2>
                <small className="text-mdGray font-light text-[16px] tracking-wider">
                  Enter your credentials to login ito administrator area.
                </small>
                <small
                  className={`text-red-500 ${isError ? "block" : "hidden"}`}
                >
                  Credentials seems to be invalid
                </small>
                <div className="flex flex-col mt-10 w-full gap-3">
                  <div className="flex flex-col w-full gap-3">
                    <label className="text-lg text-mdGray" htmlFor="email">
                      Email
                    </label>
                    <div className="relative">
                      <input
                        ...
                        name="email"
                        id="email"
                        className="w-full h-12 rounded-md outline-lightGray border border-lightGray shadow-sm relative  px-2 "
                      />
                      <span
                        className={` ${
                          isEmailValid ? "block" : "hidden"
                        } absolute top-1/2 right-4 -translate-y-1/2 text-green-500 text-2xl`}
                      >
                        <AiFillCheckCircle />
                      </span>
                  ...
                  {/* password */}
                  <div className="flex flex-col w-full gap-3">
                    <label className="text-lg text-mdGray" htmlFor="password">
                      Password
                    </label>
                    <div className="relative">
                      <input
                      ...
                      />
                      <span
                        className="absolute top-1/2 right-5 -translate-y-1/2 text-gray-500 text-2xl cursor-pointer"
                        onMouseUp={() => setIiVisible(false)}
                        onMouseDown={() => setIiVisible(true)}
                      >
                  ...
                  <input
                    ...
                    className={` w-full bg-mainBlue rounded-xl text-white p-4 mt-5 ${
                      !isEmailValid ? "!bg-lightGray" : "bg-mainBlue "
                    } `}
                  />
                </div>
              </form>

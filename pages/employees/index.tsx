...
import { deleteUserById, getAllEmployees } from "@/constants";
...
export default function Employees({ employees }: any) {
    ...
    const handleNextPagination = () => {
        if (10 * currentPage < employees.length) setCurrentPage((prev: number) => prev + 1)
    }
    //#endregion
    ...
    const handleClick = (emp: any) => {
        if (!selectedEmployees.includes(emp._id)) {
            ...
            setSelectedEmployees(selectedEmployees.filter(employee => employee != emp._id))
        }
    }
    ...
    const handleSearch = (value: string) => {
        // console.log(value)
        if (value) {
            setPageEmployees(pageEmployees.filter((emp: { firstName: string }) => emp.firstName.startsWith(value)))
        } else {
    ...
    const handleDelete = async () => {
        const deleteUsers = async () => {
            ...
            try {
                selectedEmployees.filter(id => id != user._id).forEach(async _id => {
                    await deleteUserById(_id);
                });
                setPageEmployees((prev: EmployeeType[]) => prev.filter(emp => !selectedEmployees.includes(emp._id)));
            } catch (e) {
        ...
        setModalBody("Deleteing the selected employee/s will ERASE THEM FOREVER from the database! ")
        ...
        <>
            {isLoading ? (<Loader />) : (
                <div className="flex flex-col justify-center items-center px-10">
                    ...
                    <div className="flex justify-center items-center gap-2 font-light">
                        ...
                        <p>entries</p>
                            ...
                            <div className="flex justify-center items-center">
                                {/* Search */}
                                <Search classes="rounded-lg w-[180px]" onSearch={handleSearch} />

                                {/* CRUD Operations */}
                                <Button icon={<span className="text-[#00733B] transition group-hover:text-white text-2xl"><MdOutlineAdd /></span>} title="Create" classes=" hover:bg-[#00733B] group hover:text-[white] transition" isLink={true} href={'/employees/new'} />
                                <Button
                                    icon={
                                        <span className={` text-2xl transition ${selectedEmployees.length != 1 ? ' text-darkGray group-hover:!text-darkGray' : 'text-mainBlue group-hover:!text-white'} `}>
                                            <MdModeEdit />
                                        </span>}
                                    title="Update"
                                    classes={`${selectedEmployees.length != 1 ? ' !bg-bgGray hover:!bg-bgGray ' : '!bg-lightGray hover:!bg-mainBlue hover:text-white'}  group `}
                                    isDisabled={selectedEmployees.length != 1}
                                    handleOnClick={() => router.push(`employees/${selectedEmployees[0]}`)} />
                                <Button
                                    icon={<span className={` text-2xl transition ${selectedEmployees.length < 1 ? ' text-darkGray group-hover:!text-darkGray' : '!text-[#E70C0C] group-hover:!text-white'} `}> <RiDeleteBin6Line /></span>}
                                    title="Delete"
                                    classes={`${selectedEmployees.length < 1 ? ' !bg-bgGray hover:!bg-bgGray ' : '!bg-lightGray hover:!bg-red-500 hover:text-white'}  group `}
                                    isDisabled={selectedEmployees.length < 1}
                            ...
                            <table className={` w-full`}>
                                <thead className=" bg-bgGray ">
                                    <tr className="  text-left ">
                                        <th className="">
                                            <input type="checkbox" className=" cursor-pointer" checked={selectedEmployees.length == employees.length} onClick={() => selectAll()} readOnly />
                                        </th>
                        ...
                        <div className="pagination-wrapper">
                            <div className="flex gap-5 justify-center items-center my-3">
                                <span className=" text-[#9A9A9A]  ">Showing {startingIndex == 0 ? 1 : startingIndex} to {currentPage * 10 > pageEmployees.length ? pageEmployees.length : currentPage * 10} of {pageEmployees.length} entries</span>
                                <button onClick={() => handlePrevPagination()}>&lt;</button>
                                <div className="pages">
                                    <div className="bg-[#F0F3F5] py-1 px-4 rounded-lg">{currentPage}</div>
                                </div>
                    ...
                    {/* Modal */}
                    <MyModal setIsOpen={setIsOpen} isOpen={isOpen} title={modalTitle} body={modalBody} ifTrue={modalTrue} />
                </div>

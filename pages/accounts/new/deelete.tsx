...
import { updateEmp } from "@/http/employeeHttp";

export const getServerSideProps = async (context: any) => {
  const id = context.params.id;
  const employeefetch = async () => {
    return await (await fetch(`http://34.125.207.14/employees/${id}`)).json();
  };
  const accountsFetch = async () => {
    return await (await fetch(`http://34.125.207.14/accounts`)).json();
  };
              ...
              <div className="flex flex-col justify-center items-center">
                <div className="flex items-center justify-center">
                  ...
                  <Button
                    ...
                    handleOnClick={() => deleteUser(employee.firstName)}
                  />
                </div>

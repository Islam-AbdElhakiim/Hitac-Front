...
import { revenueAccountingType } from "@/types";
...
export default function Revenue({ data }: { data: revenueAccountingType[] }) {
  ...
  const [pageData, setPageData] = useState<revenueAccountingType[]>([
    {
      type: "test",
      salesCases: "test",
      amount: "test",
  ...
  const handleDelete = async () => {
    const deleteData = async () => {
      ...
      try {
        ...
        setPageData((prev: revenueAccountingType[]) =>
    ...
    <>
      {isLoading ? (
        ...
        <div className="flex flex-col justify-center items-center px-5 h-full ">
          <PageHeader
            pageTitle="pages.revenue"
            newUrl={`/accounting/credit/revenue/new`}
          />
            ...
            <div className="flex justify-center items-center w-full  py-3">
              ...
              <div className="flex justify-center items-center">
                ...
                <Button
                  ...
                  isLink={true}
                  href={"/accounting/credit/revenue/new"}
                />
                <Button
                  icon={
                    <span
                  ...
                  handleOnClick={() => {
                    router.push(
                      `/accounting/credit/revenue/${selectedData[0]}?isEdit=true`
                    );
                  }}
                />
                <Button
            ...
            <div className="main-table w-full h-[80%] overflow-auto">
              <table className={` w-full`}>
                ...
                <tbody className="main-table overflow-auto">
                  {pageData
                    ?.filter((res: revenueAccountingType) => !res.isDeleted)
                    .map((res: revenueAccountingType, index: number) => {
                      if (index >= startingIndex && index < currentPage * 10) {
                        return (
                          <tr key={res._id} className=" text-left h-full">
                            ...
                            <td className="cursor-pointer">{res._id}</td>
                            <td className="cursor-pointer">{res.type}</td>
                            <td className="cursor-pointer">{`${res.salesCases}`}</td>
                            <td className="cursor-pointer">{`${res.amount}`}</td>
                            <td className="cursor-pointer">{`${res.account}`}</td>
                            <td className="cursor-pointer">{`${res.createdOn}`}</td>
                            <td className="cursor-pointer">{`${res.notes}`}</td>
                            <td>
                              <Link
                                href={`/accounting/credit/revenue/${res._id}`}
                              >

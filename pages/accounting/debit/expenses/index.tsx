...
import { expensesAccountingType } from "@/types";
...
export default function Expenses({ data }: { data: expensesAccountingType[] }) {
  ...
  const [pageData, setPageData] = useState<expensesAccountingType[]>([
    {
      type: "test",
      amount: "test",
      bankAccount: "test",
  ...
  const handleDelete = async () => {
    const deleteData = async () => {
      ...
      try {
        ...
        setPageData((prev: expensesAccountingType[]) =>
    ...
    <>
      {isLoading ? (
        ...
        <div className="flex flex-col justify-center items-center px-5 h-full ">
          <PageHeader
            pageTitle="pages.expenses"
            newUrl={`/accounting/debit/expenses/new`}
          />
            ...
            <div className="flex justify-center items-center w-full  py-3">
              ...
              <div className="flex justify-center items-center">
                ...
                <Button
                  ...
                  isLink={true}
                  href={"/accounting/debit/expenses/new"}
                />
                <Button
                  icon={
                    <span
                  ...
                  handleOnClick={() => {
                    router.push(
                      `/accounting/debit/expenses/${selectedData[0]}?isEdit=true`
                    );
                  }}
                />
                <Button
            ...
            <div className="main-table w-full h-[80%] overflow-auto">
              <table className={` w-full`}>
                ...
                <tbody className="main-table overflow-auto">
                  {pageData
                    ?.filter((res: expensesAccountingType) => !res.isDeleted)
                    .map((res: expensesAccountingType, index: number) => {
                      if (index >= startingIndex && index < currentPage * 10) {
                        return (
                          <tr key={res._id} className=" text-left h-full">
                            ...
                            <td>
                              <Link
                                href={`/accounting/debit/expenses/${res._id}`}
                              >

...
import { stationAccountingType } from "@/types";
...
export default function Stations({ data }: { data: stationAccountingType[] }) {
  ...
  const [pageData, setPageData] = useState<stationAccountingType[]>([
    {
      type: "test",
      station: "test",
      supplyOrder: "test",
  ...
  const handleDelete = async () => {
    const deleteData = async () => {
      ...
      try {
        ...
        setPageData((prev: stationAccountingType[]) =>
            ...
            <div className="main-table w-full h-[80%] overflow-auto">
              <table className={` w-full`}>
                ...
                <tbody className="main-table overflow-auto">
                  {pageData
                    ?.filter((res: stationAccountingType) => !res.isDeleted)
                    .map((res: stationAccountingType, index: number) => {
                      if (index >= startingIndex && index < currentPage * 10) {
                        return (
                          <tr key={res._id} className=" text-left h-full">
                            ...
                            <td className="cursor-pointer">{`${res.station}`}</td>

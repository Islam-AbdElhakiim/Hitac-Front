...
export default function BankAccounts({ data }: { data: bankAccountType[] }) {
  ...
  const [pageData, setPageData] = useState<bankAccountType[]>([
    {
      bankName: "test",
      holder: "test",
      account: "test",
  ...
  const handleDelete = async () => {
    const deleteData = async () => {
      ...
      try {
        ...
        setPageData((prev: bankAccountType[]) =>
    ...
    <>
      {isLoading ? (
        ...
        <div className="flex flex-col justify-center items-center px-5 h-full ">
          <PageHeader
            pageTitle="pages.bank_accounts"
            newUrl={`/accounting/bank-accounts/new`}
          />
            ...
            <div className="flex justify-center items-center w-full  py-3">
              ...
              <div className="flex justify-center items-center">
                ...
                <Button
                  ...
                  isLink={true}
                  href={"/accounting/bank-accounts/new"}
                />
                <Button
                  icon={
                    <span
                  ...
                  handleOnClick={() => {
                    router.push(
                      `/accounting/bank-accounts/${selectedData[0]}?isEdit=true`
                    );
                  }}
                />
                <Button
            ...
            <div className="main-table w-full h-[80%] overflow-auto">
              <table className={` w-full`}>
                ...
                <tbody className="main-table overflow-auto">
                  {pageData
                    ?.filter((res: bankAccountType) => !res.isDeleted)
                    .map((res: bankAccountType, index: number) => {
                      if (index >= startingIndex && index < currentPage * 10) {
                        return (
                          <tr key={res._id} className=" text-left h-full">
                            ...
                            <td className="cursor-pointer">{res.bankName}</td>
                            <td className="cursor-pointer">{`${res.holder}`}</td>
                            <td className="cursor-pointer">{`${res.account}`}</td>
                            <td className="cursor-pointer">{`${res.address}`}</td>
                            <td className="cursor-pointer">{`${res.available}`}</td>
                            <td>
                              <Link
                                href={`/accounting/bank-accounts/${res._id}`}
                              >

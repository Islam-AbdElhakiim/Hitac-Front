...
import { checkCookie, endpoints, getUserById } from '@/constants';
import { HIDE_LOADER, SHOW_LOADER } from '@/redux/modules/loader-slice';

const Layout = ({ children }: { children: React.ReactComponentElement<any> }) => {
    ...
    return (
        <div className="div">
            ...
            {isLocalLoading ? (<Loader />) : (
                <>
                    {component != "Login" ? (
                        <section className={` w-full h-screen rounded-3xl flex justify-center items-center px-10 gap-10 bg-bgGray ${direction == 'rtl' && 'rtl-rdirection'}`}>
                            <Navigation />
